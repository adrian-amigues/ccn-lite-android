...
char* ccnl_android_peek(char* suiteStr, char* addr, int port, char* uri) {
    ...
    // LOGE("%s sent to %s with suite %s\n", ccnl_prefix_to_path(prefix), addr, suiteStr);
    ...
    for (cnt = 0; cnt < 3; cnt++) {
        ...
        if (rc == 1 || cnt >= 2) {
            ...
            LOGE("length response = %d", strlen(response));
            return response;
        done:
            close(sock);
